...
import {
  calculateBridgeFee,
  convertCosmosBalanceToWei,
  getEthereumTransaction,
  saveEthereumTransaction,
} from "./utils";
...
const DEPOSIT_ABI = [
  "event hasBeenAdded(address sender, address recipient, string btsgBech32, uint256 amount, address contractAddress)",
];
...
function setupSigner(provider: ethers.providers.Provider): ethers.Signer {
  const signer = ethers.Wallet.fromMnemonic(cfg.EthereumMnemonic).connect(
    provider
  );
  return signer;
}
...
function setupTokenContract(readOnly = true) {
  ...
  return new ethers.Contract(
    cfg.EthereumTokenContractAddress,
    ERC20_ABI,
...
export async function parseBlock(height: number) {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    console.log(
      `Added tx to queue with id ${savedTx.id} hash: ${savedTx.hash}`
    );
    count++;
  }
  ...
  for (let tx of pendingTxs) {
    ...
    if (amountToSend > 0) {
      const receipt = await erc20.transfer(
        tx.to,
        convertCosmosBalanceToWei(amountToSend),
        txOptions
      );
      ...
      console.log(
        `Relayed ${tx.amount} from Cosmos to Ethereum. Tx: ${tx.eth_hash}`
      );
    } else {
